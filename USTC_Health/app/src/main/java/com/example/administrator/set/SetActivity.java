    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      */
        ...
        if (!mBluetoothLeService.initialize()) {//引用服务类的初始化
            Toast.makeText(SetActivity.this, "未初始化", Toast.LENGTH_LONG).show();
        }
        if(!SampleGattAttributes.connectedState) {
            if (!mBluetoothLeService.connect(MainActivity.mDeviceAddress)) {
                Toast.makeText(SetActivity.this, "未连接上", Toast.LENGTH_LONG).show();
...
reconnectsettle();
    ...
    private void  reconnectsettle(){
        if(!SampleGattAttributes.connectedState) {
            if (!mBluetoothLeService.connect(MainActivity.mDeviceAddress)) {
                Toast.makeText(SetActivity.this, "重连失败", Toast.LENGTH_LONG).show();
            }}
            else{
                SampleGattAttributes.connectedState=true;rec.setVisibility(View.GONE);
                Toast.makeText(SetActivity.this, "重连成功", Toast.LENGTH_LONG).show();
            autodisconnect();
        }
    }
    //选择测心率的周期
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //没有连接上BLE
                ...
                System.out.println("断线重连显示，并关闭判断中");
                    ...
                    .equals(action)) {
                Toast.makeText(SetActivity.this, "服务获取", Toast.LENGTH_LONG).show();
